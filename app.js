  ...
  _playSentenceDemo() {
    ...
    const speakNextChunk = () => {
      ...
      chunkEls.forEach((el, i) => {
        if (i === step) el.classList.add('active-chunk');
        else el.classList.remove('active-chunk');
      });

      window.speechCoach.speak(chunks[step], { rate: 0.75 });
      window.speechCoach.onEnd = () => {
        step++;
        setTimeout(speakNextChunk, 400); // 停頓0.4秒形成拍子
      };
    };
